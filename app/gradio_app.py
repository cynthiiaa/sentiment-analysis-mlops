...
def analyze_sentiment(text: str, enable_monitoring: bool = True):
    ...
    if enable_monitoring:
        metrics_collector.track_prediction(result, latency)
        drift_detector.add_prediction(result["confidence"], result["sentiment"])
...
def create_confidence_chart(probabilities):
    ...
    fig.update_layout(title="Sentiment Probabilities", yaxis_title="Probability", height=300)
    return fig
...
def batch_analysis(file):
    ...
    dist_fig = go.Figure(
        data=[go.Pie(labels=sentiment_dist.index, values=sentiment_dist.values, hole=0.3)]
    )
    dist_fig.update_layout(title="Sentiment Distribution")

    conf_fig = go.Figure(data=[go.Histogram(x=results_df["confidence"], nbinsx=20)])
    conf_fig.update_layout(
        title="Confidence Distribution", xaxis_title="Confidence", yaxis_title="Count"
    )
...
with gr.Blocks(title="MLOps Sentiment Analysis", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Single Prediction"):
        with gr.Row():
            with gr.Column():
                ...
                monitor_checkbox = gr.Checkbox(label="Enable Monitoring", value=True)
    ...
    with gr.Tab("Batch Analysis"):
        ...
        batch_btn.click(
            fn=batch_analysis, inputs=file_upload, outputs=[results_df, dist_chart, conf_chart]
        )
